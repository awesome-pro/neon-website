...
subtitle: Learn how to set usage quotas per project with the Unique API
enableTableOfContents: true
isDraft: false
updatedOn: '2024-08-06T15:23:10.949Z'
---

When setting up your billing solution with Neon, you may want to impose some hard limits on how much storage or compute resources a given project can consume. For example, you may want to cap how much usage your free plan users can consume versus pro or enterprise users. With the Unique API, you can use the `quota` key to set usage limits for a variety of consumption metrics. These limits act as thresholds after which all active computes for a project are [suspended](#suspending-active-computes).

## Metrics and quotas

By default, Unique tracks a variety of consumption metrics at the project level. If you want to set quotas (max limits) for these metrics, you need to explicitly [configure](#configuring-quotas) them.
...
Unique updates all metrics every 15 minutes but it could take up to 1 hour before they are reportable.
...
- `data_transfer_bytes` &#8212; Sets the maximum amount of egress data, measured in bytes, that can be transferred out of Unique from across all of a project's branches using the proxy.
...
### Unique default limits

In addition to the configurable limits that you can set, Unique also sets certain branch size limits by default. You might notice these limits in a [Get Project](#retrieving-details-about-a-project) response:
...
The Unique Free Plan logical data size limit is approximately 512 MiB:

- `branch_logical_size_limit`: 512
- `branch_logical_size_limit_bytes`: 536870912

The Unique Launch and Scale plan failsafe limit is 200 GiB:
...
If you need to change the quota limits for an existing project &#8212; for example, if a user switches their plan to a higher usage tier &#8212; you can reset those limits via `PATCH` request. See [Update a project](https://api-docs.neon.tech/reference/updateproject) in the Unique API.
...
You can use the Unique API to retrieve consumption metrics for your organization and projects through various endpoints:
