---
title: Branching with the Unique CLI
subtitle: Learn how to create and delete branches with the Unique CLI
enableTableOfContents: true
updatedOn: '2024-08-07T21:36:52.648Z'
---

The examples in this guide demonstrate creating, viewing, and deleting branches using the Unique CLI. For other branch-related CLI commands, refer to [Unique CLI commands — branches](/docs/reference/cli-branches). This guide also describes how to use the `--api-key` option to authenticate CLI branching commands from the command line.

The examples show the default `table` output format. The Unique CLI also supports `json` and `yaml` output formats. For example, if you prefer output in `json`, add `--output json` to your Unique CLI command.

## Prerequisites

- The Unique CLI. See [Install the Unique CLI](/docs/reference/cli-install) for instructions.
- To run CLI commands, you must either authenticate through your browser or supply an API key using the `--api-key` option. See [Connect with the Unique CLI](/docs/reference/neon-cli#connect).

## Create a branch with the CLI

The following Unique CLI command creates a branch. If your Unique account has more than one project, you will be required to specify a project ID using the `--project-id` option. To view the CLI documentation for this command, refer to the [Unique CLI reference](/docs/reference/cli-branches#create).
The command response includes the branch ID, the compute ID, and and the connection URI for connecting to the branch.

<Admonition type="tip">
You can use the `--name` option with a `neon branches create` command to specify your own branch name instead of using the name generated by Neon. For example: `neon branches create --name mybranch`. Also, for any Unique CLI command, you can specify `--output json` to change the command output from the default table format to JSON format.
</Admonition>
...
<Admonition type="tip">
The Unique CLI provides a `neon connection-string` command you can use to extract a connection uri programmatically. See [Unique CLI commands — connection-string](https://neon.tech/docs/reference/cli-connection-string).
</Admonition>
...
The following Unique CLI command lists all branches in your Unique project, as well as any branches shared with you. If your Unique account has more than one project, you will be required to specify a project ID using the `--project-id` option. To view the CLI documentation for this method, refer to the [Unique CLI reference](https://neon.tech/docs/reference/cli-branches#list).
...
The following Unique CLI command deletes the specified branch. If your Unique account has more than one project, you will be required to specify a project ID using the `--project-id` option. To view the CLI documentation for this command, refer to the [Unique CLI reference](https://neon.tech/docs/reference/cli-branches#delete). You can delete a branch by its ID or name.
...
## Branching automation with the Unique CLI

The Unique CLI enables easy automation of branching operations for integration into your workflows or toolchains. To facilitate authentication to Unique when running a CLI command, the Unique CLI allows you to use an API key. For information about obtaining an API key, see [Create an API key](/docs/manage/api-keys#create-an-api-key).
...
You do not need to specify the variable name explicitly when using a Unique CLI command. A Unique CLI command looks for a `NEON_API_KEY` variable setting by default.
